...
# Pattern: Consolidated MCP tool pattern (find_tasks, manage_task)
# Extracted: 2025-10-06
# Relevance: 10/10 - Core MCP implementation pattern

"""
Consolidated task management tools for MCP Server.

KEY PATTERNS TO MIMIC:
1. Consolidated tool pattern: find_tasks (list/search/get), manage_task (create/update/delete)
2. Response optimization for MCP: truncate large fields, replace arrays with counts
3. Structured error handling with MCPErrorFormatter
...
DEFAULT_PAGE_SIZE = 10  # Reduced from 50 for MCP responses
...
def optimize_task_response(task: dict) -> dict:
    ...
    Single task GET operations can return full details.
    """
    ...
    # Replace arrays with counts (reduces payload size)
...
def register_task_tools(mcp: FastMCP):
    ...
    async def find_tasks(
        ctx: Context,
        query: str | None = None,  # Keyword search capability
        ...
        - query provided: Searches title/description/feature
        ...
        """
        try:
            # Single task get mode - return full details without optimization
            if task_id:
            ...
            # PATTERN: Route selection based on filter type
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                # Normalize response format (handle different API response shapes)
                ...
                # CRITICAL: Optimize task responses for MCP
    ...
    async def manage_task(
        ...
        Examples:
          manage_task("create", project_id="p-1", title="Research existing patterns")
