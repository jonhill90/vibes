...
# Lines: Full file (combined best practices)
# Pattern: JSONL manifest logging for phase tracking and audit trail
...
#   - jq with grep fallback (no external dependencies required)
#
# What to Adapt:
#   - Additional metadata fields (model, tokens, etc.)
#   - Log file location based on your directory structure
#   - Manifest path pattern (currently: prps/{feature}/codex/logs/manifest.jsonl)
#
# What to Skip:
#   - Pretty-printing (JSONL must be one-line-per-entry)
#   - Complex nested structures (keep flat for grep/jq parsing)
#
# CRITICAL GOTCHA: Concurrent Writes
#   - Multiple parallel agents writing to same manifest = RACE CONDITION
#   - Solution: Atomic writes via temp file + mv (shown in log_phase_* functions)
...
# Validate feature name (security check - 6 levels)
validate_feature_name() {
    ...
    # Level 4: Command injection chars
    local dangerous_chars='$`;&|><'
    if [[ "$feature" =~ [$dangerous_chars] ]]; then
        echo "❌ Dangerous characters in feature name: $feature" >&2
        return 1
    fi

    # Level 5: Newline/carriage return
    if [[ "$feature" == *$'\n'* || "$feature" == *$'\r'* ]]; then
        echo "❌ Newline characters in feature name: $feature" >&2
        return 1
    fi

    # Level 6: Empty check
    if [ -z "$feature" ]; then
        echo "❌ Empty feature name" >&2
        return 1
    fi
