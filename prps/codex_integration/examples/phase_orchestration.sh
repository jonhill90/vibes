#!/bin/bash
# Source: .claude/commands/generate-prp.md Phase 0-4 + parallel execution pattern
# Pattern: Multi-phase orchestration with parallel execution support
# Extracted: 2025-10-07
# Relevance: 10/10 - Core orchestration pattern for PRP workflows
...
show_progress() {
    ...
    for phase in "${!PHASES[@]}"; do
        if source "${OUTPUT_DIR}/scripts/manifest_logger.sh" && \
           validate_phase_completion "$phase" &>/dev/null; then
    ...
    echo ""
    echo "Progress: ${completed}/${total_phases} phases (${percentage}%)"
    echo "========================================="
}
