...
class MCPMiddlewareRemote:
    ...
    def __init__(self):
        self.loaded_mcps = {}
        self.terraform_loaded = False  # Simple flag for terraform tools
    ...
    def _setup_tools(self):
        ...
        async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
            ...
            elif name.startswith("terraform_"):
                logger.info(f"🎉 SUCCESS! Called terraform tool: {name}")
                return [TextContent(
                    type="text", 
                    text=f"🎉 PROOF OF CONCEPT SUCCESS! Tool aggregation is working! Called {name} with {arguments}. This proves terraform tools are dynamically available after load_mcp!"
                )]
            else:
    ...
    async def _load_mcp(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            # Set terraform flag to add tools
            if name == "terraform":
                self.terraform_loaded = True
                logger.info(f"Setting terraform_loaded = True")
            ...
            return [TextContent(
                type="text",
                text=f"Successfully loaded MCP '{name}' with command '{command}'. Terraform tools should now be available!"
            )]
        except Exception as e:
            return [TextContent(
                type="text",
